...
def deve_enviar(hoje, regra):
    ...
    if dia_semana in r:
        return True
    if "DIA 12 OU ÚTIL" in r and (hoje.day == 12 or (hoje.weekday() < 5 and hoje.day < 12)):
        return True
    return False
...
def main():
    ...
    for linha in dados:
        try:
            ...
            if not deve_enviar(agora, regra): continue
            horario = linha[idx["Horário"]]
            if ":" not in horario: continue
            h, m = map(int, horario.strip().split(":"))
            hora_regra = h + m / 60
            if abs(hora_atual - hora_regra) > 0.25: continue
